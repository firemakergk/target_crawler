...
class RssCrawler:
    ...
    def crawlRss(self):
        ...
        for t in cur:
            ...
            dataVal = (t[0], rssJson , jsonMd5, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
            cur1 = self.conn.cursor()
            cur1.execute('select count(*) from target_mapping where target_id = %s',(t[0],))
            count = cur1.fetchone()
            cur1.close()
            if count[0] > 0:
                self.conn.cursor().execute('update target_mapping set items=%s,md5=%s,update_time=%s where target_id=%s', (rssJson,jsonMd5,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())),t[0]))
            else:
